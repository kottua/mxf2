export function calculateOnboardingPrice(unitData, specData, config, incomePlans) {
    ...
    const sortedPlans = [...incomePlans].sort((a, b) => new Date(a.period_begin).getTime() - new Date(b.period_begin).getTime());
    let basePrice = 0;
...
export const calculateSoldout = (specData, config) => {
    if (!Array.isArray(specData)) {
